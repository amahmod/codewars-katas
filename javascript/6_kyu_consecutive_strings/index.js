...
function longestConsec(array, k) {
  if (k > array.length || array.length === 0 || k <= 0) return ''

  let longestString = ''

  for (let i = 0, len = array.length; i < len - k + 1; i += 1) {
    const newStr = array.slice(i, i + k).join('')
    if (newStr.length > longestString.length) longestString = newStr
  }
